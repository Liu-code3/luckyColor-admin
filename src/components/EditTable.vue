...
<template>
  <div class="tableContainer">
    <table class="table">
      ...
      <tbody>
        <tr v-for="(row, index) in matchDataArr" :key="index">
          ...
          >
            <component
              :is="getComponentType(item.loaderType)"
              v-model="row[item.field]"
              :options="item.children"
              @change="handleChange(row)"
              @input="handleInput($event, index, item.field)"
              @click.stop="selectFn"
            >
              <!-- 如果是展示组件，这里用插槽展示内容 -->
              <template v-if="['FLOW_SELECTOR_TYPEDISABLED', 'FLOW_DISABLES_TYPE', 'DISPLAY_DIV', 'DISPLAY_SPAN'].includes(item.loaderType)">
                {{ row[item.field] }}
              </template>
            </component>
          </td>
