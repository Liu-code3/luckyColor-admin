...
- 启动命令：`pnpm dev`
- 请求前缀：`/api`
- Vite 代理目标：`VITE_API_PROXY_TARGET=http://127.0.0.1:3001`
- 实际效果：浏览器请求 `/api/*`，开发服务器转发到真实后端 `http://127.0.0.1:3001/*`
...
- 请求前缀：`/api`
- Vite 代理目标：关闭
- 实际效果：浏览器请求 `/api/*`，已覆盖接口直接由本地 `mock/` 返回数据

### 混合模式

- 启动命令：`pnpm dev:hybrid`
- 请求前缀：`/api`
- Vite 代理目标：`VITE_API_PROXY_TARGET=http://127.0.0.1:3001`
- 实际效果：已覆盖接口走本地 Mock，未覆盖接口继续代理到真实后端
